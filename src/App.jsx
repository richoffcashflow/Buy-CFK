...
export default function App(){
  ...
  async function checkPayment(quiet=false){
    try{
      ...
      if(data.status==='completed'){
        ...
        const updated=await api(`/position?wallet=${walletBridge.current.address}`);setPosition(updated);
        if(data.direction==='onramp'&&walletAction==='buy'&&Number.isFinite(updated.availableUsd))setAmount((Math.floor(Math.min(Number(amount),updated.availableUsd)*100)/100).toFixed(2));
        setFlow({step:data.direction==='onramp'?'funded':'paid'});
  ...
  return <>
    ...
    {modal==='transaction'&&<Modal title={walletAction==='withdraw'?'Cash out':walletAction==='fund'?'Add money':walletAction==='sell'?'Sell $CFK':'Buy $CFK'} onClose={close}>
      ...
      {flow.step==='funded'&&<div className="flow-state"><Icon name="shield"/><h3>Your money is ready</h3><p>{walletAction==='buy'?'Your buy amount now reflects the money available after fees. Review your coins before confirming.':'You can now choose your amount and buy $CFK.'}</p><button className="primary" onClick={()=>walletAction==='buy'?prepare('buy',walletBridge.current):close()}>{walletAction==='buy'?'Review buy':'Back to $CFK'}</button></div>}
