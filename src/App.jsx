...
export default function App(){
  ...
  async function resume(){
    ...
    try{
      ...
      if(result.status==='sandbox_processing'){setFlow({step:'sandbox-processing'});return;}
  ...
  useEffect(()=>{if(!account||!['checkout','pending','sandbox-processing'].includes(flow.step))return;const t=setInterval(()=>resume(),5000);return()=>clearInterval(t);},[account,flow.step]);
  ...
  return <>
    ...
    {modal==='transaction'&&<Modal title={flow.step==='auth-error'?'Sign in to continue':intent==='withdraw'?'Withdraw cash':intent==='sell'?'Sell CFK':'Buy CFK'} onClose={close}>
      {flow.step==='auth-error'&&<div className="flow-state"><h3>Let’s try signing in again</h3><p role="status">{flow.message}</p><button className="primary" onClick={()=>signIn('telegram')}>Continue with Telegram</button></div>}
      {['sign-in','idle','loading','buying','confirming'].includes(flow.step)&&<div className="flow-state"><div className="spinner"/><h3>{flow.step==='sign-in'?'Signing you in':flow.step==='idle'?'Getting you ready':flow.step==='confirming'?'Confirming your transaction':flow.step==='buying'?(flow.side==='sell'?'Selling your CFK':'Buying your CFK'):'Preparing your amount'}</h3><p>{flow.step==='sign-in'?'Connecting your Telegram account…':flow.step==='idle'?'A quick sign-in keeps your coins yours.':'You can follow the progress here.'}</p></div>}
      ...
      {flow.step==='sandbox-processing'&&<div className="flow-state"><h3>Test payment accepted</h3><p>Stripe is still processing the test delivery. This is not a completed CFK purchase. No real money moves in this sandbox.</p><p>You can close this window. Your pending test is saved; there is no need to pay again.</p><button className="secondary" disabled={busy} onClick={resume}>Check test status</button><button className="secondary" onClick={close}>Back to CFK</button></div>}
