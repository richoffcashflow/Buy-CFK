...
export async function authenticate(req,wallet){
  if(!process.env.PRIVY_APP_ID||!process.env.PRIVY_APP_SECRET||!process.env.PRIVY_VERIFICATION_KEY)throw appError('Wallet setup is being completed. Please try again later.',503);
  const token=String(req.headers.authorization||'').replace(/^Bearer /,'');
  if(!token)throw appError('Open your wallet to continue.',401);
  try{
  ...
  }catch(e){if(e.status)throw e;throw appError('Your session expired. Open your wallet again.',401);}
}
